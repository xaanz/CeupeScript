...
// @updateURL    https://github.com/xaanz/CeupeScript/raw/main/BetterD.user.js
// @downloadURL  https://github.com/xaanz/CeupeScript/raw/main/BetterD.user.js
...
// Saltos de línea después de las etiquetas de Firma y Puesto
const labelRemitente = document.querySelector('label[for="txtRemitente"]');
if (labelRemitente) {
    const br1 = document.createElement('br');
    const br2 = document.createElement('br');
    labelRemitente.parentNode.insertBefore(br1, labelRemitente.nextSibling);
    labelRemitente.parentNode.insertBefore(br2, br1.nextSibling);
}

    const labelPuesto = document.querySelector('label[for="txtPuesto"]');
    if (labelPuesto) {
        const br1 = document.createElement('br');
        const br2 = document.createElement('br');
        labelPuesto.parentNode.insertBefore(br1, labelPuesto.nextSibling);
        labelPuesto.parentNode.insertBefore(br2, br1.nextSibling);
    }

    // Espera a que el DOM esté cargado
    function moverDivDerecha() {
        const selectPlantillas = document.getElementById('vm-selector-plantillas');
        const divBotones = document.getElementById('htmltxtComunicacion$HtmlEditorExtenderBehavior_ExtenderButtonContainer');

        if (!selectPlantillas || !divBotones) return;

        // Crear un contenedor padre para ambos elementos si no existe
        let contenedor = selectPlantillas.parentElement;

        // Si ya están juntos, solo aplicamos estilos
        // Si no, los agrupamos en un contenedor flex
        if (!contenedor.classList.contains('contenedor-flex')) {
            const wrapper = document.createElement('div');
            wrapper.className = 'contenedor-flex';
            wrapper.style.display = 'flex';
            wrapper.style.alignItems = 'center';
            wrapper.style.gap = '12px'; // espacio entre select y div

            // Insertamos el wrapper antes del select
            contenedor.insertBefore(wrapper, selectPlantillas);
            // Movemos select y div dentro del wrapper
            wrapper.appendChild(selectPlantillas);
            wrapper.appendChild(divBotones);
        }

        // Opcional: ajustar estilos para que el div quede alineado a la derecha del select
        divBotones.style.marginLeft = 'auto'; // empuja el div a la derecha dentro del flex
    }

// 1. Firma automática no editable
const spanUsuario = document.getElementById("headerTutorizacion_usuarioSesion_lblUsuario");
const nombreUsuario = spanUsuario ? spanUsuario.textContent.trim() : "Su Docente";
const NUEVO_TEXTO = "\n\n" + nombreUsuario;
        ...
        if (match) {
            ...
            infoDiv.innerHTML = `
                ...
                <img src="https://flagcdn.com/48x36/${countryData[match].flag}.png"
                     ...
                     width="48" height="36" alt="Bandera de ${match}">
